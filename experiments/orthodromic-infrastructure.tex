...
\label{sec:general-position-counts}
...
These numbers are upper bounds for arrangements in which multiway concurrence is allowed. When several circles pass through the same antipodal pair, vertices and cells merge. The adopted nine-orthodrome architecture, specified in full in Section~\ref{sec:nine-orthodrome-realization}, is precisely such a case: it is constructed so that several circles deliberately concur, and its true vertex, edge, and cell counts are correspondingly far smaller than the general-position maxima above. Section~\ref{sec:concurrent-cubic-realization} derives these counts directly.
...
\label{sec:nine-orthodrome-realization}
...
\section{The Concurrent Cubic-Symmetric Realization}
\label{sec:concurrent-cubic-realization}

The general-position formula of Section~\ref{sec:general-position-counts} gives \(V=72\), \(E=144\), \(F=74\) for nine circles, but these are upper bounds attained only when no three circles concur. The five fixed reference orthodromes already violate this assumption at the poles, and the cyclic construction of Section~\ref{sec:nine-orthodrome-realization} admits a special case in which the four flexible orthodromes concur maximally as well. This section derives the resulting counts explicitly, since they describe the specific architecture adopted elsewhere in this work rather than a generic configuration.

Let the cyclic generator \(R\in SO(3)\) of Section~\ref{sec:nine-orthodrome-realization} be the \(90^\circ\) rotation about the shared polar axis already used to define the equator and the meridian family, and let the seed normal be
...
corresponding to \(\beta_6=45^\circ\), \(\lambda_6=0^\circ\) in the parameterization of equation~\eqref{eq:flexible-normal}. Applying \(R^k\) for \(k=1,2,3\), and invoking the projective identification \(n\sim -n\) already noted, gives

\begin{equation}
    n_7=Rn_6=\frac{1}{\sqrt2}\begin{pmatrix}0\\1\\1\end{pmatrix},
    \qquad
    n_8=R^2n_6\sim\frac{1}{\sqrt2}\begin{pmatrix}1\\0\\-1\end{pmatrix},
    \qquad
    n_9=R^3n_6\sim\frac{1}{\sqrt2}\begin{pmatrix}0\\1\\-1\end{pmatrix}.
\end{equation}

Together with the five fixed normals \(n_1=(0,0,1)\), \(n_2=(1,0,0)\), \(n_3=(0,1,0)\), \(n_4=(1,1,0)/\sqrt2\), \(n_5=(-1,1,0)/\sqrt2\) already introduced, the full set \(\{n_1,\dots,n_9\}\) is exactly the nine mirror planes of the full octahedral symmetry group: the three coordinate planes, together with all six diagonal planes \(x=\pm y\), \(y=\pm z\), \(x=\pm z\). No arbitrary choice remains once \(R\) and the seed are fixed to the polar axis: the adopted five-orthodrome reference set and this cyclic four-orthodrome family jointly realize the complete reflection arrangement of the cube.

\begin{proposition}
The concurrent arrangement of the nine cubic mirror planes has
\[
V=26,
\qquad
E=72,
...
\begin{proof}
Every point fixed by at least two of the nine reflections is a vertex of the arrangement, and by the symmetry of the cube these vertices fall into exactly three orbits under the group of order \(48\): the eight corner directions \((\pm1,\pm1,\pm1)/\sqrt3\), the twelve edge-midpoint directions given by permutations of \((\pm1,\pm1,0)/\sqrt2\), and the six face-center directions \((\pm1,0,0)\), \((0,\pm1,0)\), \((0,0,\pm1)\). Direct evaluation shows that exactly three of the nine planes pass through each corner, exactly two pass through each edge midpoint, and exactly four pass through each face center. Since every pair of planes meets at exactly one antipodal pair of points, summing \(\binom{m}{2}\) over the antipodal vertex classes must recover \(\binom{9}{2}=36\): four corner pairs contribute \(4\binom32=12\), six edge pairs contribute \(6\binom22=6\), and three face pairs contribute \(3\binom42=18\), and \(12+6+18=36\) confirms that no vertex has been omitted. The vertex count is therefore \(V=8+12+6=26\).

Each vertex of multiplicity \(m\) has degree \(2m\), since each of the \(m\) circles through it contributes two arc-ends. Summing degree over all vertices gives \(8(2\cdot3)+12(2\cdot2)+6(2\cdot4)=48+48+48=144\), and since this sum equals \(2E\), \(E=72\). Euler's formula then gives
\[
F=2-V+E=2-26+72=48.
\]
\end{proof}
...
\begin{remark}
The eight corners, twelve edge midpoints, and six face centers of this arrangement correspond exactly to the eight corner pieces, twelve edge pieces, and six center pieces of a Rubik's cube. The coincidence is not superficial: both objects are organized by the same order-\(48\) octahedral symmetry group acting on the same three orbit types. The nine orthodromes of this realization are precisely the nine mirror planes that generate that group, so the arrangement graph's vertex classes and the physical cube's piece classes are two descriptions of one underlying structure.
\end{remark}

This resolves the concurrent case of the research question posed above for exact finite symmetry: constraining the four flexible orthodromes to the cyclic orbit of a single seed under a \(90^\circ\) rotation about the existing polar axis, with that seed chosen at \(\beta_6=45^\circ\), does not merely impose \emph{a} symmetry group but the unique one under which every one of the nine orthodromes participates in a shared, maximally concurrent incidence structure. Sections concerned with the flexible orthodromes as a free optimization variable, including the objective vector \(\mathbf J(\theta)\) and the admissible set \(\Theta_{\mathrm{adm}}\) of the following chapter, should treat this configuration as the distinguished point at which \(C_{\mathrm{sym}}=0\) and at which the degeneracy penalty \(C_{\mathrm{dup}}\) is evaluated at its structurally intended, rather than accidental, concurrences.

Because the cyclic generator \(R\) admits many valid choices of rotation axis and seed, only one of which reproduces the cubic-symmetric arrangement above, the nine normals are given here as an explicit, self-contained reference implementation rather than left to be reconstructed from the prose description alone. Listing~\ref{lst:nine-normals} computes all nine normals directly from their closed-form values, enumerates the pairwise plane intersections, and reports the resulting vertex multiplicity classes, so that any independent implementation can be checked against the exact \(V=26\), \(E=72\), \(F=48\) counts of the preceding proposition rather than against a verbal description of the construction.

\begin{lstlisting}[caption={Reference generator for the nine cubic-symmetric orthodrome normals, their pairwise intersections, and vertex multiplicity classification.}, label={lst:nine-normals}]
import numpy as np
import itertools

sqrt2 = np.sqrt(2)

# The nine orthodrome normals in the paper's convention (Z = polar axis).
# n1..n5 are the fixed reference set; n6..n9 are the flexible orthodromes,
# realized here at the unique cyclic-symmetric configuration of
# Section "The Concurrent Cubic-Symmetric Realization".
normals = {
    "n1_equator":        np.array([0, 0, 1]),
    "n2_meridian_lam0":  np.array([1, 0, 0]),
    "n3_meridian_lam90": np.array([0, 1, 0]),
    "n4_meridian_x=y":   np.array([1, 1, 0]) / sqrt2,
    "n5_meridian_x=-y":  np.array([-1, 1, 0]) / sqrt2,
    "n6_flex_x=z":       np.array([1, 0, -1]) / sqrt2,
    "n7_flex_x=-z":      np.array([1, 0, 1]) / sqrt2,
    "n8_flex_y=z":       np.array([0, 1, -1]) / sqrt2,
    "n9_flex_y=-z":      np.array([0, 1, 1]) / sqrt2,
}
names = list(normals.keys())
N = [normals[k] for k in names]

# All pairwise great-circle intersections (each pair meets at one
# antipodal vertex-pair on the unit sphere).
vertices = []
for i, j in itertools.combinations(range(9), 2):
    v = np.cross(N[i], N[j])
    if np.linalg.norm(v) < 1e-9:
        continue          # would indicate coincident planes; none occur here
    v = v / np.linalg.norm(v)
    for sign in (1, -1):
        p = sign * v
        if not any(np.linalg.norm(np.array(q) - p) < 1e-9 for q in vertices):
            vertices.append(tuple(p))

# Classify each vertex by how many of the nine planes pass through it.
multiplicity = {}
for v in vertices:
    v = np.array(v)
    m = sum(1 for n in N if abs(np.dot(n, v)) < 1e-9)
    multiplicity[m] = multiplicity.get(m, 0) + 1

assert len(vertices) == 26
assert multiplicity == {3: 8, 2: 12, 4: 6}   # corners, edges, face centers

print(f"V = {len(vertices)}")            # 26
print(f"multiplicity classes: {multiplicity}")   # {3: 8, 2: 12, 4: 6}
\end{lstlisting}

Running this listing reproduces \(V=26\) with the vertex classes \(\{8 \text{ at multiplicity } 3,\ 12 \text{ at multiplicity } 2,\ 6 \text{ at multiplicity } 4\}\) claimed in the proposition above, independent of any particular choice of software, coordinate library, or three-dimensional modeling tool used to visualize the arrangement.
